...
csv_files = human_files.merge(vcm_files, on=['segment_onset', 'segment_offset', 'filename'], # add suffixes for clarity
                             suffixes=('_human', '_vcmnet'), how='inner') 

# below for when adding in meg_classifier
# csv_files = human_files.merge(vcm_files, on=['segment_onset', 'segment_offset', 'filename'], # add suffixes for clarity
#                              suffixes=('_human', '_vcmnet'))\
#                        .merge(meg_files, on=['segment_onset', 'segment_offset', 'filename'], 
#                              suffixes=('', '_meg'), how='outer')
...
labels = sorted(list(set(csv_files['vcm_type_human'].dropna()) | 
                     set(csv_files['vcm_type_vcmnet'].dropna())))
...
for human_label, vcm_label in zip(csv_files['vcm_type_human'], csv_files['vcm_type_vcmnet']):                          
    if pd.notna(human_label) and pd.notna(vcm_label):
        if human_label in labels and vcm_label in labels:
