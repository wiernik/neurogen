...
def main(argv):
    ...
    if (result_path / 'gamma.csv').is_file():
        out = pd.read_csv(result_path / 'gamma.csv')
    ...
    for recording_file in tqdm(recording_files):
        if recording_file not in processed:
    ...
    print(f"Results can be found in {result_path / 'gamma.csv'}")
    print("Done.")
